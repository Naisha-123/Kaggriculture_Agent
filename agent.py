...
def my_agent(obs):
    ...
    # Cap milk sales per turn — dumping everything crashes our own market
    # once 7 cows are compounding; wool/tomato volume is low enough to sell freely
    milk_amt = shed.get("MILK", 0)
    if milk_amt > 0:
        market_orders.append(["SELL", "MILK", min(milk_amt, 3)])
    for product in ("WOOL", "TOMATO"):
